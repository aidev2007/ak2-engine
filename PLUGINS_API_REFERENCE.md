...
└── src/
    ...
    └── plugins/
        ├── layout-header-nav/       # グローバルヘッダー（Nav + Drawer + Toggle）
        ...
        ├── block-button-wave/       # SVG ウェーブ遷移ボタン
        ├── block-card-kinetic/      # キネティック 3D ティルトカード
        ├── layout-hero-particle/    # テキストパーティクル ヒーロー
        ├── effect-cursor-magnetic/  # カスタムカーソル + 磁力エフェクト
...
**どんな表示になるか:**
ページ上部に固定されるグローバルヘッダー。PC ではインタラクティブなグローバルナビ（5バリアント）、モバイルではトグルボタン（4バリアント）＋ドロワーメニュー（4バリアント）が表示されます。すべてのスタイルはフロントマターから指定可能で、省略時はデフォルト値が適用されます。

**フロントマターパラメータ（すべてオプション）:**
...
| `navBg` | `"glass"` | `"solid"` — 完全不透明白背景（暗いヒーロー面に重ねる場合に指定） |
...
`base.njk` にインクルードされており、直接呼び出す必要はありません。

**グローバルナビ バリアント一覧:**

| 値 | 説明 |
|---|---|
| `"underline"` | アクティブ項目に下線インジケーターが追従（滑らかなスライド） |
...
| 値 | 説明 |
|---|---|
| `"circular"` | トグルボタン起点の円形リビール（`navTogglePos` に連動） |
...
- スクロール 10px 超で `is-scrolled` クラスを付与（ヘッダー縮小 + 背景強化）
- `transparentHeader: true` で初期状態が透明（ヒーロー画像との重ね使い）
- `navTogglePos: "left"` 時は `circular` ドロワーの展開起点も左上に切り替わる
- ドロワー展開時にスクロールバー幅を自動補正（レイアウトシフト防止）
...
| クラス | 説明 |
|---|---|
| `.ak2-nav-header` | ヘッダーベース（`position: fixed`） |
| `.ak2-nav-header.is-scrolled` | スクロール後の縮小・背景強化状態 |
| `.ak2-nav-header.is-transparent` | ページトップ透明状態 |
| `.ak2-nav-header.toggle-pos-left` | トグル左配置状態 |
| `.ak2-nav-global` | PC グローバルナビ（768px 以下で非表示） |
| `.ak2-nav-global--{variant}` | グローバルナビのバリアントクラス |
| `.ak2-nav-drawer` | ドロワーベース（`position: fixed`, 全画面） |
| `.ak2-nav-drawer--{variant}` | ドロワーのバリアントクラス |
| `.ak2-nav-drawer.is-open` | ドロワー展開状態 |
| `.ak2-nav-toggle` | トグルボタンベース |
| `#ak2-nav-toggle-wrap` | トグルラッパー（768px 超で非表示） |
...
### `layout-hero-particle` — `heroParticle`

**どんな表示になるか:**
テキストをパーティクル（粒子）に分解して描画するフルビューポートのヒーローセクション。Canvas 上に無数の小さな点でテキストを構成し、マウスが近づくと粒子が反発し、離れると元の形に戻ります。ダーク背景に `--color-primary` のパーティクルが映える演出です。スマホ（`hover: none`）は CSS フォールバックでテキストを直接表示します。
...
| `extraClass` | string | `""` | `<section>` への追加クラス |

> **ヒント:** 暗い背景の上にヘッダーが重なる場合は `navBg: "solid"` をフロントマターに追加してください。
...
**どんな表示になるか:**
ホバーすると 3D 視点で傾き（パースペクティブティルト）、光の反射（グレア）、画像のパララックス効果が発生するカード。マウスの位置に合わせてリアルタイムに傾きが変化し、マウスアウトで滑らかに元に戻ります。ダークテーマにも対応。タッチデバイスは 3D エフェクト無効・通常表示になります。
...
{# グリッドレイアウト（3カラム auto-fit）#}
{% call kineticCardGrid() %}
  {% call kineticCard(title="Card 1", img="/img/1.jpg") %}内容1{% endcall %}
  {% call kineticCard(title="Card 2", img="/img/2.jpg") %}内容2{% endcall %}
  {% call kineticCard(title="Card 3", img="/img/3.jpg") %}内容3{% endcall %}
{% endcall %}
```
...
| `dark` | boolean | `false` | ダークテーマ（濃紺背景・白テキスト） |

**kineticCardGrid パラメーター:**

| パラメーター | 型 | デフォルト | 説明 |
|---|---|---|---|
| `minWidth` | string | `"300px"` | カード最小幅（`auto-fit` 制御） |
| `gap` | string | `"40px"` | カード間のギャップ |

> **動作確認:** `/interaction/` ページで 3D ティルトを実際にホバーして確認できます。
...
**どんな表示になるか:**
クリックすると画面下から波形の SVG が上昇して全画面を覆い、指定した href へ遷移します。波が上がりきった後に遷移するため、ページ切り替えが流体的な演出になります。`href="#"` を指定するとアニメーションのみ再生してリセットするデモモードになります（GSAP 不要・純粋な Canvas/SVG のみ）。
...
{{ waveButton("Contact", "/contact/", color="#0d9488", textColor="#0d9488") }}
```
...
| `color` | string | `"var(--color-primary)"` | ウェーブ（波）の色 |
| `textColor` | string | `"var(--color-primary)"` | ボタンテキスト・枠線の色 |

> **動作確認:** `/interaction/` ページでウェーブアニメーションを実際にクリックして確認できます。
...
### `effect-cursor-magnetic` — `cursorMagnetic`

**どんな表示になるか:**
`data-cursor-area` 属性を付与したセクション内に入ると、デフォルトのカーソルが非表示になり、小さな点（ドット）と輪（リング）の2層構造のカスタムカーソルに切り替わります。ドットは即座に追従し、リングは Lerp（線形補間）でなめらかに遅れて追従します。`data-magnetic` 属性を付与した要素はカーソルに吸い付く磁力エフェクトが発生します。エリア外では通常カーソルに戻ります。

**セットアップ（body 内に1回のみ配置）:**
...
```html
<!-- このセクション内でカスタムカーソルが有効になる -->
<section data-cursor-area>
  <!-- data-magnetic を付与した要素はカーソルに吸い付く -->
  <button data-magnetic type="button">Magnetic Button</button>
  <a href="/about/" data-magnetic>Magnetic Link</a>
</section>
```
...
| CSS 変数 | デフォルト | 説明 |
|---|---|---|
| `--color-primary` | — | ドット色・リング色（`base.css` の変数を使用） |

> **注意:** `cursorMagnetic()` マクロ呼び出しは 1 ページあたり 1 回のみにしてください。

> **動作確認:** `/interaction/` ページのデモエリア内でカーソルの変化を確認できます。
...
| `/menubar/` | `layout-header-nav` 全バリアントのリファレンス＋動作確認 |
| `/menubar_demo/` | `layout-header-nav` インタラクティブPlayground（独立HTML） |
| `/concept/` | `article.njk` レイアウトのデモ（記事ページ） |
| `/interaction/` | カーソル・パーティクルヒーロー・キネティックカード・ウェーブボタンの動作確認 |
...
*このドキュメントは `@ak2lab/engine v0.1.3` に対応しています。*
